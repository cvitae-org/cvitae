 ...
 *
 * Two readers sit behind this. cvitae's own `fetchOffer` is a plain GET and
 ...
 *                          fall back and behave as cvitae always did.
...
import { scrapeOffer, type BoardOffer } from './scraperClient';
...
export const resolveOffer = async (url: string): Promise<ResolvedOffer> => {
  ...
  const outcome = await fetchOffer(url);
