 ...
 * not listening, the research route falls back to the built-in `fetchOffer` and
 * behaves exactly as it did before — starting it is an upgrade, not a
...
export const scrapeOffer = async (url: string): Promise<ScraperOutcome> => {
  const base = baseUrl();
